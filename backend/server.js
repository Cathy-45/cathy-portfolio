...
app.post('/api/consultations', async (req, res) => {
  ...
  db.query(query, [name, email, phone, message], (err, result) => {
    ...
    res.status(201).json({ message: 'Consultation request submitted successfully', data: result });
  });
});
...
app.post('/api/payments', async (req, res) => {
  ...
  try {
    ...
    // Update consultations table with payment intent
    const query = 'UPDATE consultations SET payment_intent_id = ?, payment_status = ? WHERE email = ? AND name = ? ORDER BY created_at DESC LIMIT 1';
    db.query(query, [session.payment_intent, session.payment_status, email, name], (err) => {
      if (err) {
